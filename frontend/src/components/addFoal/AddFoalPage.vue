<template>
  <v-form v-model="valid" class="form-container" validate-on="input" @submit.prevent="createFoal">
    ...
    <div class="button-container">
      <v-btn class="final-button" color="green" type="submit">veulen toevoegen</v-btn>
  ...
  methods: {
    createFoal() {
      if(this.valid && this.newFoal.birthDate.length > 1){
        this.foalStore.addFoal(this.newFoal);
        setTimeout(() => {
          this.backToFoals();
        }, 500)
      }
